...
Scenario: Should be able to load a config file from the file system
  Given a new config parser
  Given a config file ./config.yml with content {"foo": "bar"}
...
Scenario: Should be able to merge two config file
  Given a new config parser
  Given a config file ./config.yml with content {"foo": "bar"}
  Given a config file ./config2.yml with content {"bar": "foo"}
  When ./config.yml is loaded
  When ./config2.yml is loaded
  Then config is {"foo": "bar", "bar": "foo"}

Scenario: A config file include another config file case
  Given a new config parser
  Given a config file ./config.yml with content {"foo": "bar", "bar2": "foo2"}
  Given a config file ./folder/config2.yml with content {"include":["../config.yml"], "bar": "foo", "foo": "new bar"}
